...
private:
    ...
    void StartPortalLogin(const std::string& username, const std::string& password);
    static void WifiEventHandler(void* arg, esp_event_base_t event_base, int32_t event_id, void* event_data);
    static void IpEventHandler(void* arg, esp_event_base_t event_base, int32_t event_id, void* event_data);
    
    // Portal login state
    std::string pending_portal_username_;
    std::string pending_portal_password_;
    bool needs_portal_login_ = false;
};
