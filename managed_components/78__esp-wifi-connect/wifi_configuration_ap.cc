...
void WifiConfigurationAp::StartWebServer()
{
    ...
    for (const auto& url : captive_portal_urls) {
        httpd_uri_t redirect_uri_get = {
            .uri = url,
            .method = HTTP_GET,
            .handler = captive_portal_handler,
            .user_ctx = this
        };
        ESP_ERROR_CHECK(httpd_register_uri_handler(server_, &redirect_uri_get));

        httpd_uri_t redirect_uri_post = {
            .uri = url,
            .method = HTTP_POST,
            .handler = captive_portal_handler,
            .user_ctx = this
        };
        ESP_ERROR_CHECK(httpd_register_uri_handler(server_, &redirect_uri_post));
    }
...
bool WifiConfigurationAp::ConnectToWifi(const std::string &ssid, const std::string &password, const std::string &username)
{
    ...
    // Determine if we should use Enterprise Authentication
    bool use_enterprise = false;
    if (!username.empty()) {
        // Check finding SSID in AP records to see auth mode
        std::lock_guard<std::mutex> lock(mutex_);
        bool ssid_found = false;
        for (const auto& ap : ap_records_) {
            if (ssid == reinterpret_cast<const char*>(ap.ssid)) {
                ssid_found = true;
                if (ap.authmode == WIFI_AUTH_WPA2_ENTERPRISE || 
                    ap.authmode == WIFI_AUTH_WPA3_ENTERPRISE || 
                    ap.authmode == WIFI_AUTH_WPA2_WPA3_ENTERPRISE) {
                    use_enterprise = true;
                }
                break;
            }
        }
        // If the SSID is hidden or not found in scan (manual entry), assume Enterprise if username is present
        // This maintains compatibility with hidden Enterprise networks
        if (!ssid_found) {
            use_enterprise = true;
        }
    }
    ...
    if (use_enterprise) {
        ESP_LOGI(TAG, "Configuring WPA2 Enterprise for %s", ssid.c_str());
        ...
        if (!username.empty()) {
            ESP_LOGI(TAG, "Configuring Portal Login for %s (Username provided but not Enterprise)", ssid.c_str());
        }
...
{
    ESP_LOGI(TAG, "Save SSID %s %d, username %s", ssid.c_str(), ssid.length(), username.c_str());
